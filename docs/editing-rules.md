...
- Always retain the exact Scan original. Select a strict, metadata-free,
  browser-portable JPEG source directly when possible; otherwise generate one
  correctly oriented readability derivative suitable for A4 pages.
- The implemented Scan pipeline accepts browser-compatible JPEG, PNG, or WebP
  originals up to 20,000,000 bytes, verifies signatures and full decode, records
  SHA-256, and rejects exact duplicates against registered originals and only
  those readability JPEGs that remain stored. A safe source under 1 MiB never
  enters the encoder. For a safe source of at least 1 MiB, retain the optional
  quality-85/2400 candidate only if it saves at least 20% and 256 KiB. Format,
  dimensions, orientation, metadata, transparency, or compatibility concerns
  require a metadata-free white-background quality-85 JPEG no larger than 2400
  pixels per edge. Required failure fails safely; optional failure uses the
  already safe source. The authenticated viewer follows the immutable
  readability selection and `Open original` always exposes the exact source.
