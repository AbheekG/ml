...
- `recordings` stores one required, normalized-unique per-Song description,
  optional recorded date and contributors, processing state, an original media
  object, and an optional playback object. Recorded dates use the shared India
  calendar in the client, Worker, and D1 triggers; D1 computes its maximum as
  UTC+05:30 rather than UTC. Imported Version and the four populated Notes are
  combined losslessly for display and also remain in hidden `legacy_*` columns.
...
- `recording_upload_sessions`, `recording_upload_intents`, `recording_upload_parts`, and `recording_upload_credits` durably retain an editor-owned multipart request, its immutable create/replace target, a canonical file manifest, Worker-measured part hashes, R2-returned part ETags, intended metadata, revisions, and terminal outcome. Finalization rechecks byte-exact duplicates against current and historical Recording originals and generated playback representations, then atomically creates or updates the exact Recording only when no match exists, preserves replacement history, copies credits, and creates the policy-bound pending job. A same-Recording historical match can atomically restore the retained pair while archiving the displaced current pair. Private object keys, multipart IDs, ETags, and generated catalog/job IDs are never browser inputs; client hash claims are independently verified.
...
- Recording source replacement is blocked while processing is active. Scan/Recording replacement history is immutable, and exact retained historical originals can be made current again only through revision-guarded same-child operations that first archive the displaced media.
