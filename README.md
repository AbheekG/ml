...
- the local Worker implementation now also has a separately authenticated audio-processor claim/lease boundary, operation-scoped source/derivative/result/failure capabilities, immutable per-attempt derivative upload, independent stored-byte verification, atomic ready/provenance finalization, safe failure checkpointing, and explicit editor retry; a provider-neutral local Python adapter can claim and process at most one job with strict origin, redirect, byte-integrity, retry, and temporary-file controls, but its invocation/server wrapper, processor secret/origin, scheduling, browser orchestration, and staging deployment are intentionally not configured yet;
