...
- the local Worker implementation now also has a separately authenticated audio-processor claim/lease boundary, operation-scoped source/derivative/result/failure capabilities, immutable per-attempt derivative upload, independent stored-byte verification, atomic ready/provenance finalization, safe failure checkpointing, and explicit editor retry; a provider-neutral local Python adapter can claim and process one job with strict origin, redirect, byte-integrity, retry, and temporary-file controls; the accepted hosted-boundary design uses a scheduled single-task Cloud Run Job with no HTTP server; its isolated Google Cloud staging project, billing link, budget alerts, and local CLI configuration now exist, but its concurrency/deadline prerequisites, processor secret/origin, runtime resources, browser orchestration, and staging deployment are intentionally not configured yet;
