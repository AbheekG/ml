...
- the local Worker implementation now also has a separately authenticated audio-processor claim/lease boundary, operation-scoped source/derivative/result/failure capabilities, immutable per-attempt derivative upload, independent stored-byte verification, atomic ready/provenance finalization, safe failure checkpointing, and explicit editor retry; a provider-neutral local Python adapter can claim and process one job with strict origin, redirect, byte-integrity, retry, and temporary-file controls; the proposed hosted boundary is a scheduled single-task Cloud Run Job with no HTTP server, but its concurrency/deadline prerequisites, processor secret/origin, scheduler, cloud resources, browser orchestration, and staging deployment are intentionally not configured yet;
