...
The subsequent archive-maintenance rollout removes the builder-owned resumable
cache only after a verified build, identifies failed/interrupted work for
optional manual cleanup, separates active Songs from Trash in the HTML preview,
and leaves an empty Song visible without a broken folder link.
...
- capable online browsers can share the authenticated, policy-selected readable
  representation for an individual Scan directly from its Song row or viewer;
  this can be either a validated direct JPEG source or a retained derivative.
  Exported filenames use the Song title, Scan metadata, and stable multi-Scan
  position without exposing the original basename; public URLs and persistent
  media caches are not involved;
- private Recording playback sharing is deployed with a Recording-scoped ready-MP3 route, a 50 MiB safety bound, and no client-selected storage identifier or public URL; exported MP3 names use the Song title and Recording description without exposing the original basename; the owner accepted the semantic filename and principal sharing behavior on a real device, while the deliberately oversized and slow-download paths remain automatically covered rather than manually forced;
...
- the current protected-staging deployment is Worker
  ...
  the private archive/in-progress work cache outside Git repositories. The
...
- editors/admins can create and replace private JPEG, PNG, or WebP Scans up to
  the Cloudflare Images 20 MB input limit. The Worker verifies signatures,
  SHA-256, and full decode and always retains the exact source. A strict bounded
  metadata-free JPEG is used directly; a source under 1 MiB is never re-encoded,
  and an optional candidate for a larger safe source is kept only when it saves
  at least 20% and 256 KiB. Format/dimension/metadata/orientation/transparency
  concerns require a verified quality-85/2400 JPEG. Exact duplicates cover
  retained originals and retained derivatives with race-safe D1 guards;
...
- imported and newly uploaded Scans use a private selected-representation read
  path; a bounded daily repair task backfills fingerprints and readability
  selections with expiring per-media leases and privacy-safe failure records.
  Originals remain private and retained;
