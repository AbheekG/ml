...
- the Worker has a durable, editor-owned 8 MiB multipart intake for creating or replacing private Recording originals. Immutable upload intents bind each session to its exact operation, byte-exact duplicate checks cover both retained originals and generated playback representations, recovery controls expose resumable/stored/duplicate sessions without exposing storage identifiers, replacement preserves prior media history, and active processing/upload guards prevent conflicting Trash or source changes;
