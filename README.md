...
- capable online browsers can share the authenticated optimized JPEG for an individual Scan directly from its Song row or from the viewer through the native system share sheet; exported Scan filenames use the Song title, Scan metadata, and stable multi-Scan position without exposing the original basename; original Scan bytes, public URLs, and persistent media caches are not involved, and the owner accepted the resulting semantic filename behavior on a real device;
