...
- capable online browsers can share the authenticated optimized JPEG for an individual Scan directly from its Song row or from the viewer through the native system share sheet; original Scan bytes, public URLs, and persistent media caches are not involved, and Android/iPadOS acceptance of the row action remains pending;
- Song-row Scan actions use accessible eye, connected-share, and pencil symbols with text on wider layouts and 44-pixel icon-only touch targets on narrow layouts;
